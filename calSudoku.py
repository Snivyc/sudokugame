...
class Solution(object):
    ...
    # 解出点(x,y)的结果，如果解出，则解(next_x,next_y)的结果；解不出或(next_x,next_y)无法顺利填完，则回溯到上一层
    def solve(self, x, y):
        if self.count > 1:  # 如果count > 1，则该数独题目解的数量不唯一，直接退出所有递归。
            return
        ...
        for i in range(1, 10):  # 将1-9尝试填入当前的空格
            if self.check(x, y, i):  # 如果可以填入，则填入
                self.sudoku[y][x] = i
                # print(self.sudoku)
                if next_x == -1:  # 如果已经填完，则count增加1
                    # print(self.sudoku)
                    self.count = self.count + 1
                    self.sudoku[y][x] = 0
                else:
                    self.solve(next_x, next_y)  # 如果未填完，填下一个空
